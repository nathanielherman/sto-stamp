...
def run_genstm(bench, config):
  os.chdir('./' + bench)
  subprocess.check_output(['make', '-f', 'Makefile.STO', 'clean'], stderr=subprocess.STDOUT)
  subprocess.check_output(['make', '-f', 'Makefile.STO'], stderr=subprocess.STDOUT)
  cmd = ['./' + bench]
  cmd.extend(config.split())
  out = subprocess.check_output(cmd)
  #print out
...
if __name__ == "__main__":
  ...
  for b in bench:
    ...
    genstmstat = run_genstm(b, configs[b])
